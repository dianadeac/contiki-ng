...
def main():
    ...
    for i in range(1, num_runs+1):
        ...
        success = execute_test(input_file)
        if success:
            new_log = f"COOJA-{i}.testlog"
            try:
                shutil.move(cooja_output, new_log)
                print(f"Saved output to {new_log}")
            except Exceptipn as e:
                print(f"Could not save output for run {i}: {e}")
        else:
